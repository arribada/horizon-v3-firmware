...
static void apply_config(nrfx_uarte_t const * p_instance, nrfx_uarte_config_t const * p_config)
{
...
int syshal_uart_init(uint32_t instance)
{
    ...
    apply_config(&UART_Inits[instance].uarte, &UART_Inits[instance].uarte_config);
...
int syshal_uart_read_timeout(uint32_t instance, uint8_t * buffer, uint32_t buf_size, uint32_t read_timeout_us, uint32_t last_char_timeout_us, uint32_t * bytes_received)
{
    ...
    *bytes_received = 0;
    ...
    while (bytes_left_to_read)
    {
        ...
        {
            //syshal_pmu_sleep(SLEEP_LIGHT); // NOTE: It is safer not to sleep so that the soft watchdog remains running
        }
    }
