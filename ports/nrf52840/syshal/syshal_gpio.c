...
{
    uint32_t pin_number;
    void (*callback)(syshal_gpio_event_t);
        ...
        if (syshal_gpio_irq_conf[ch].pin_number == pin)
        {
            if (syshal_gpio_irq_conf[ch].callback)
            {
                if (NRF_GPIOTE_POLARITY_LOTOHI)
                    syshal_gpio_irq_conf[ch].callback(SYSHAL_GPIO_EVENT_LOW_TO_HIGH);
                else if (NRF_GPIOTE_POLARITY_HITOLO)
                    syshal_gpio_irq_conf[ch].callback(SYSHAL_GPIO_EVENT_HIGH_TO_LOW);
                else if (NRF_GPIOTE_POLARITY_TOGGLE)
                    syshal_gpio_irq_conf[ch].callback(SYSHAL_GPIO_EVENT_TOGGLE);
            }
            break;
        }
    }
