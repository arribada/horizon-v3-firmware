...
{
    uint32_t seconds;
    syshal_rtc_date_time_to_timestamp(date_time, &seconds);

    // Disable the RTC
    nrfx_rtc_disable(&RTC_Inits[RTC_TIME_KEEPING].rtc);

    // Reset the RTC counter back to zero
    nrfx_rtc_counter_clear(&RTC_Inits[RTC_TIME_KEEPING].rtc);

    // Calculate the rtc offset and number of overflows that have occured for this date/time
    rtc_offset = ((seconds % SECONDS_PER_OVERFLOW) * RTC_TIME_KEEPING_FREQUENCY_HZ) & 0xFFFFFF;
    overflows_occured = seconds / SECONDS_PER_OVERFLOW;

    // Re-enable the RTC
    nrfx_rtc_enable(&RTC_Inits[RTC_TIME_KEEPING].rtc);
    nrfx_rtc_overflow_enable(&RTC_Inits[RTC_TIME_KEEPING].rtc, true); // This may not be needed as it is called in syshal_rtc_init()?
...
int syshal_rtc_get_timestamp(uint32_t * timestamp)
{
    ...
    *timestamp = ((nrfx_rtc_counter_get(&RTC_Inits[RTC_TIME_KEEPING].rtc) + rtc_offset)) / RTC_TIME_KEEPING_FREQUENCY_HZ + (overflows_occured * SECONDS_PER_OVERFLOW);
