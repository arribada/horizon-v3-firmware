...
static void amts_evt_handler(nrf_ble_amts_evt_t evt)
{
    ...
    switch (evt.evt_type)
    {
        case NRF_BLE_AMTS_EVT_NOTIF_ENABLED:
        {
            ...
            if (m_ble_params.conn_interval != CONN_INTERVAL_DEFAULT)
            {
        ...
        {
            //DEBUG_PR_TRACE("RX Complete.");

            if (!rx_pending)
            {
                DEBUG_PR_ERROR("BLE Data received before syshal_ble_receive() was called. Data lost!");
                return;
            }

            if (evt.receive_complete.length > rx_length)
            {
                DEBUG_PR_ERROR("BLE Data received but buffer for it too small. Data lost!");
                return;
            }

            // Copy the received data into the receive buffer
            memcpy(rx_buffer, evt.receive_complete.buffer, evt.receive_complete.length);
            ...
            syshal_ble_event_t event =
            {
                ...
                .receive_complete = {
                    .length = evt.receive_complete.length
                }
            };
...
static void advertising_data_set(void)
{
    ...
    ble_gap_adv_params_t adv_params =
    {
        ...
        .interval      = m_ble_params.advertising_interval,
        .duration      = 0,
...
int syshal_ble_init(void)
{
    ...
    if (sys_config.tag_bluetooth_device_address.hdr.set)
    {
        ble_gap_addr_t address;

        for (uint32_t i = 0; i < BLE_GAP_ADDR_LEN; ++i)
            address.addr[i] = sys_config.tag_bluetooth_device_address.contents.address[i];
...
int syshal_ble_send(uint8_t *buffer, uint32_t size)
{
    ...
    tx_pending = true;
    tx_length = size;
    return nrf_ble_amts_send(&m_amts, buffer, size);
}
