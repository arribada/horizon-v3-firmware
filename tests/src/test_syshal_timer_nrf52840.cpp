...
void timer_callback(void) { timer_triggered.push_back(current_time); }
std::deque<uint32_t> wakeup_event;
void syshal_rtc_wakeup_event(void) { wakeup_event.push_back(current_time); }
...
TEST_F(Syshal_TimerTestNRF52840, ChangeTimeBackwards5Seconds)
{
    ...
    ASSERT_EQ(1, timer_triggered.size());
    EXPECT_EQ(0, timer_triggered.front());
    ASSERT_EQ(0, wakeup_event.size());
}
...
TEST_F(Syshal_TimerTestNRF52840, ChangeTimeForward5Seconds)
{
    ...
    ASSERT_EQ(1, timer_triggered.size());
    EXPECT_EQ(25, timer_triggered.front());
    ASSERT_EQ(1, wakeup_event.size());
    EXPECT_EQ(25, wakeup_event.front());
}
