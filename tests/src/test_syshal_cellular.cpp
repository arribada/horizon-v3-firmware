...
public:
    ...
    void read_buffer_simple_mock(uint8_t *buffer, uint32_t len, int status = SYSHAL_CELLULAR_NO_ERROR)
    {
        ...
        syshal_uart_read_timeout_ExpectAndReturn(uart_device_priv, NULL, len, SYSHAL_CELLULAR_TIMEOUT_MS * MICROSECOND, char_timeout_microsecond, &bytes_received, status);
    ...
    void Inject_mock_syshal_cellular_dettach(int fail_step, int inject_error_code, uint32_t timeout)
    {
        int error_code = 0;
        int step_counter = 0;
    ...
    void Inject_mock_syshal_cellular_read_from_file_to_fs(int fail_step, int inject_error_code, fs_handle_t handle)
    {
        ...
        if (++step_counter == fail_step)
            error_code = inject_error_code;
        expect_simple_mock(expected_respond_read_file, SYSHAL_CELLULAR_TIMEOUT_MS, error_code);
        if (error_code) return;
...
{
    Inject_mock_syshal_cellular_setcom(0, SYSHAL_CELLULAR_NO_ERROR);
    int status = syshal_cellular_sync_comms();
...
TEST_F(CellularTest, syshal_cellular_check_sim_success)
{
    ...
    /* Act */
    int status = syshal_cellular_check_sim(imsi);
...
TEST_F(CellularTest, syshal_cellular_create_secure_profile_success)
{
    ...
    /* Act */
    int status = syshal_cellular_create_secure_profile();
...
TEST_F(CellularTest, syshal_cellular_set_rat_succes)
{
    ...
    /* Act */
    int status = syshal_cellular_set_rat(timeout, SCAN_MODE_2G);
...
TEST_F(CellularTest, syshal_cellular_scan_succes)
{
    ...
    /* Act */
    int status = syshal_cellular_scan(timeout);
...
TEST_F(CellularTest, syshal_cellular_attach_succes)
{
    ...
    /* Act */
    int status = syshal_cellular_attach(timeout);
...
/*                                                                  */
/*              syshal_cellular_dettach                            */
/*                                                                  */
TEST_F(CellularTest, syshal_cellular_dettach_succes)
{
    /* Arrange */
    uint32_t timeout = 1000;
    Inject_mock_syshal_cellular_dettach(0, SYSHAL_UART_NO_ERROR, timeout);

    /* Act */
    int status = syshal_cellular_dettach(timeout);
...
TEST_F(CellularTest, syshal_cellular_dettach_fail_flush )
{
    /* Arrange */
    uint32_t timeout = 1000;
    Inject_mock_syshal_cellular_dettach(1, SYSHAL_UART_ERROR_DEVICE, timeout);
...
TEST_F(CellularTest, syshal_cellular_dettach_fail_send)
{
    /* Arrange */
    uint32_t timeout = 1000;
    Inject_mock_syshal_cellular_dettach(2, SYSHAL_UART_ERROR_DEVICE, timeout);
...
TEST_F(CellularTest, syshal_cellular_dettach_fail_expect)
{
    /* Arrange */
    uint32_t timeout = 1000;
    Inject_mock_syshal_cellular_dettach(3, SYSHAL_UART_ERROR_TIMEOUT, timeout);
...
TEST_F(CellularTest, syshal_cellular_activate_pdp_no_user_success)
{
    ...
    /* Act */
    int status = syshal_cellular_activate_pdp(&apn, timeout);
...
TEST_F(CellularTest, syshal_cellular_activate_pdp_user_success)
{
    ...
    /* Act */
    int status = syshal_cellular_activate_pdp(&apn_user, timeout);
...
TEST_F(CellularTest, syshal_cellular_get_HTTP_success)
{
    ...
    /* Act */
    int status = syshal_cellular_https_get(timeout, domain, port, path);
...
TEST_F(CellularTest, syshal_cellular_post_HTTP_success)
{
    ...
    /* Act */
    int status = syshal_cellular_https_post(timeout, domain, port, path);
