...
int syshal_ble_init_GTest(int cmock_num_calls)
{
...
void (*GPIO_callback_function[GPIO_TOTAL_NUMBER])(syshal_gpio_event_t);
...
int syshal_gpio_enable_interrupt_GTest(uint32_t pin, void (*callback_function)(syshal_gpio_event_t), int cmock_num_calls) {GPIO_callback_function[pin] = callback_function; return SYSHAL_GPIO_NO_ERROR;}
...
class Sm_MainTest : public ::testing::Test
{
    ...
    virtual void SetUp()
    {
        ...
        // syshal_button
        syshal_button_init_IgnoreAndReturn(SYSHAL_BUTTON_NO_ERROR);
        syshal_button_term_IgnoreAndReturn(SYSHAL_BUTTON_NO_ERROR);
    ...
    virtual void TearDown()
    {
        ...
        Mocksyshal_button_Verify();
        Mocksyshal_button_Destroy();
...
public:
    ...
    void boot_operational_gps_test(void)
    {
        ...
        req.cmd_test_req.test_device_flag = CMD_TEST_REQ_GPS_BIT;
        ...
        sys_config.gps_trigger_mode.contents.mode = SYS_CONFIG_GPS_TRIGGER_MODE_SCHEDULED;
    ...
    void boot_operational_cellular_test(void)
    {
        ...
        cmd_t req;
        CMD_SET_HDR((&req), CMD_TEST_REQ);
        req.cmd_test_req.test_device_flag = CMD_TEST_REQ_CELLULAR_BIT;
        syshal_usb_queue_message((uint8_t *) &req, CMD_SIZE(cmd_test_req_t));
    ...
    void boot_operational_gps_cellular_test(void)
    {
        ...
        cmd_t req;
        CMD_SET_HDR((&req), CMD_TEST_REQ);
        req.cmd_test_req.test_device_flag = (CMD_TEST_REQ_GPS_BIT | CMD_TEST_REQ_CELLULAR_BIT);
        syshal_usb_queue_message((uint8_t *) &req, CMD_SIZE(cmd_test_req_t));
    ...
    void SetGPIOPin(uint32_t pin, bool state)
    {
        ...
        if (GPIO_callback_function[pin])
            GPIO_callback_function[pin](SYSHAL_GPIO_EVENT_TOGGLE);
    }
...
TEST_F(Sm_MainTest, OperationalStateGPSAlwaysOnIoTTrigger)
{
    auto min_gps_updates = 100;
    ...
    // Set GPS acquisition interval
    sys_config.gps_scheduled_acquisition_interval.hdr.set = true;
    sys_config.gps_scheduled_acquisition_interval.contents.seconds = 0;

    // Set GPS acquisition time
    sys_config.gps_maximum_acquisition_time.hdr.set = true;
    sys_config.gps_maximum_acquisition_time.contents.seconds = 0;

    // Set GPS no fix timeout
    sys_config.gps_scheduled_acquisition_no_fix_timeout.hdr.set = true;
    sys_config.gps_scheduled_acquisition_no_fix_timeout.contents.seconds = 0;
    ...
    // Progress some time just to make sure the IoT isn't triggered
    ...
    syshal_gps_event_t gps_event;
    gps_event.event_id = SYSHAL_GPS_EVENT_STATUS;
    gps_event.event_data.status.gpsFix = 2;
    syshal_gps_callback(gps_event);

    gps_event.event_id = SYSHAL_GPS_EVENT_POSLLH;

    // Generate 1 less than the number of required GPS updates to trigger an IoT connection
    for (auto i = 0; i < min_gps_updates - 1; ++i)
        syshal_gps_callback(gps_event);

    sm_tick(&state_handle);

    // Generate the final GPS update required to trigger the IoT layer
    syshal_gps_callback(gps_event);
    sm_iot_trigger_ExpectAndReturn(IOT_RADIO_CELLULAR, SM_IOT_NO_ERROR);
    sm_tick(&state_handle);

    // Progress some time just to make sure the IoT isn't triggered again
    for (auto i = 0; i < 2000; ++i)
    {
        IncrementSeconds(1);
        sm_tick(&state_handle);
...
TEST_F(Sm_MainTest, OperationalStateGPSScheduledIoTMaxBackoff)
{
    uint32_t acquisition_interval = 165;
    uint32_t maximum_acquisition = 15;
    uint32_t no_fix_timeout = 0;
    ...
    sm_iot_event.code = SM_IOT_NO_ERROR;
    sm_iot_callback(&sm_iot_event);
...
TEST_F(Sm_MainTest, DISABLED_OperationalStateGPSScheduledFirstFixHold)
{
    uint32_t acquisition_interval = 165;
    uint32_t maximum_acquisition = 15;
    uint32_t no_fix_timeout = 0;
    uint32_t test_fix_hold_time = 60;
    ...
    for (unsigned int i = 0; i < acquisition_interval + maximum_acquisition + test_fix_hold_time + 1; ++i)
    {
        IncrementSeconds(1);
    ...
    for (unsigned int i = 0; i < test_fix_hold_time + 1; ++i)
    {
        IncrementSeconds(1);
        syshal_gps_callback(event);
        sm_tick(&state_handle);
...
TEST_F(Sm_MainTest, DISABLED_OperationalStateGPSScheduledFirstFixHoldLostFix)
{
    uint32_t acquisition_interval = 165;
    uint32_t maximum_acquisition = 15;
    uint32_t no_fix_timeout = 0;
    ...
    for (unsigned int i = 0; i < (test_fix_hold_time / 2) + 1; ++i)
    {
        IncrementSeconds(1);
        syshal_gps_callback(event);
        sm_tick(&state_handle);
    ...
    for (unsigned int i = 0; i < (test_fix_hold_time); ++i)
    {
        IncrementSeconds(1);
        syshal_gps_callback(event);
        sm_tick(&state_handle);
...
TEST_F(Sm_MainTest, StatusRequestUSB)
{
    ...
    device_id_t id_nordic;
    for (int i = 0; i < sizeof(device_id_t); ++i)
    {
        id_nordic[i] = 0;
    }
    ...
    syshal_cellular_is_present_ExpectAndReturn(celluar_is_present);
    syshal_cellular_check_sim_ExpectAndReturn(imsi, SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_check_sim_ReturnArrayThruPtr_imsi(imsi, imsi_length_sara);
    syshal_cellular_check_sim_IgnoreArg_imsi();
    syshal_device_id_ExpectAndReturn(&id_nordic, SYSHAL_DEVICE_NO_ERROR);
    syshal_device_id_ReturnArrayThruPtr_device_id( &id_nordic, sizeof(id_nordic));
    syshal_device_id_IgnoreArg_device_id();
    config_if_backend_t backend = {.id = CONFIG_IF_BACKEND_USB};
    config_if_init(backend);
    USBConnectionEvent();
    ...
    EXPECT_EQ(APP_FIRMWARE_VERSION, resp.cmd_status_resp.stm_firmware_version);
    EXPECT_EQ(syshal_ble_version, resp.cmd_status_resp.ble_firmware_version);


    EXPECT_TRUE( 0 == std::memcmp( id_nordic, resp.cmd_status_resp.nordic_uid, sizeof( resp.cmd_status_resp.nordic_uid ) ) );
    ...
    EXPECT_EQ(SYS_CONFIG_FORMAT_VERSION, resp.cmd_status_resp.configuration_format_version);
}
...
TEST_F(Sm_MainTest, FsScriptConfigurationUpdateSuccessful)
{
    ...
    buffer[0] = ((uint16_t) SYS_CONFIG_TAG_TEMP_SENSOR_LOG_ENABLE);
    buffer[1] = ((uint16_t) SYS_CONFIG_TAG_TEMP_SENSOR_LOG_ENABLE) >> 8;
...
TEST_F(Sm_MainTest, TestGPS)
{
    ...
    boot_operational_gps_test();

    sm_tick(&state_handle);

    // Increment to get past the initial 5 seconds of green LED flashing
    ...
    for (unsigned int i = 0; i < acquisition_interval + maximum_acquisition + test_fix_hold_time + 1; ++i)
    {
        ...
        EXPECT_EQ(SYSHAL_LED_COLOUR_WHITE, colour);
        EXPECT_TRUE(is_blinking);
    }
    // Generate a GPS fixed event
    event.event_id = SYSHAL_GPS_EVENT_STATUS;
    event.event_data.status.gpsFix = 2;
...
TEST_F(Sm_MainTest, TestCellular)
{
    bool is_blinking;
    uint32_t colour;
    ...
    sm_iot_trigger_force_ExpectAndReturn(IOT_RADIO_CELLULAR, 0);
...
TEST_F(Sm_MainTest, TestGPSCellular)
{
    uint32_t acquisition_interval = 165;
    uint32_t maximum_acquisition = 15;
    uint32_t no_fix_timeout = 0;
    ...
    boot_operational_gps_cellular_test();
