...
int syshal_cellular_check_sim_callback(uint8_t * imsi, int cmock_num_calls)
{
...
int syshal_cellular_https_get_callback(uint32_t timeout, const char *domain, uint32_t port, const char *path, int cmock_num_calls)
{
...
int syshal_cellular_https_post_callback(uint32_t timeout, const char *domain, uint32_t port, const char *path, int cmock_num_calls)
{
...
int syshal_cellular_read_from_file_to_fs_callback(fs_handle_t handle, uint32_t * http_code, uint32_t * file_size, int cmock_num_calls)
{
...
public:
    ...
    void default_iot_init_config()
    {
        ...
        // Artic settings
        for (auto i = 0; i < sizeof(iot_sat_artic_settings.contents.device_identifier); ++i)
            iot_sat_artic_settings.contents.device_identifier[i] = random_range(0U, UINT8_MAX);
        for (auto i = 0; i < 8; ++i)
        {
            iot_sat_artic_settings.contents.bulletin[i].satellite_code = i + 1;
            iot_sat_artic_settings.contents.bulletin[i].seconds_since_epoch = random_range(0U, UINT8_MAX);
            for (auto j = 0; j < 6; ++j)
                iot_sat_artic_settings.contents.bulletin[i].params[j] = random_range(-100.0f, 100.0f);
        }
    ...
    void cellular_power_on()
    {
        ...
        syshal_cellular_sync_comms_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
        syshal_cellular_create_secure_profile_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
    ...
    void cellular_connect()
    {
        ...
        syshal_cellular_scan_ExpectAndReturn(connect_timeout, SYSHAL_CELLULAR_NO_ERROR);
        syshal_cellular_attach_ExpectAndReturn(connect_timeout, SYSHAL_CELLULAR_NO_ERROR);
        syshal_cellular_activate_pdp_ExpectAndReturn(((syshal_cellular_apn_t*)&iot_cellular_apn.contents), connect_timeout, SYSHAL_CELLULAR_NO_ERROR);
    ...
    std::vector<uint8_t> generate_log_data(size_t min_length)
    {
        ...
        {
            uint16_t tag_id = logging_tag_lookup[random_range(0U, LOGGING_COUNT - 1)];
            size_t tag_size;
...
TEST_F(IOTTest, powerOnCellularBackendPowerOnFailure)
{
    ...
    EXPECT_EQ(IOT_ERROR_BACKEND, iot_power_on(IOT_RADIO_CELLULAR));
}
...
TEST_F(IOTTest, powerOnCellularBackendSyncCommsFailure)
{
    EXPECT_EQ(IOT_NO_ERROR, iot_init(iot_init_config));
    syshal_cellular_power_on_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_sync_comms_ExpectAndReturn(-1);
    EXPECT_EQ(IOT_ERROR_BACKEND, iot_power_on(IOT_RADIO_CELLULAR));
}
...
TEST_F(IOTTest, powerOnCellularBackendCreateProfileFailure)
{
    ...
    syshal_cellular_power_on_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_sync_comms_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_create_secure_profile_ExpectAndReturn(-1);
    EXPECT_EQ(IOT_ERROR_BACKEND, iot_power_on(IOT_RADIO_CELLULAR));
}
...
TEST_F(IOTTest, powerOnCellularcheckSimNotPresent)
{
    ...
    syshal_cellular_power_on_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_sync_comms_ExpectAndReturn(SYSHAL_CELLULAR_NO_ERROR);
    EXPECT_EQ(IOT_ERROR_NO_SIM_FOUND, iot_power_on(IOT_RADIO_CELLULAR));
}
...
TEST_F(IOTTest, checkSimNotPresent)
{
    ...
    syshal_cellular_check_sim_return_val = SYSHAL_CELLULAR_ERROR_SIM_CARD_NOT_FOUND;
    EXPECT_EQ(IOT_ERROR_NO_SIM_FOUND, iot_check_sim(&imsi));
}
...
TEST_F(IOTTest, checkSimBackendError)
{
    ...
    syshal_cellular_check_sim_return_val = -1;
    EXPECT_EQ(IOT_ERROR_BACKEND, iot_check_sim(&imsi));
}
...
TEST_F(IOTTest, connectScanTimeout)
{
    EXPECT_EQ(IOT_NO_ERROR, iot_init(iot_init_config));
    cellular_power_on();
    syshal_cellular_scan_ExpectAndReturn(CONNECT_TIMEOUT, SYSHAL_CELLULAR_ERROR_TIMEOUT);
    EXPECT_EQ(IOT_ERROR_NOT_RADIO_COVERAGE, iot_connect(CONNECT_TIMEOUT));
}
...
TEST_F(IOTTest, connectAttachTimeout)
{
    ...
    cellular_power_on();
    syshal_cellular_scan_ExpectAndReturn(CONNECT_TIMEOUT, SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_attach_ExpectAndReturn(CONNECT_TIMEOUT, SYSHAL_CELLULAR_ERROR_TIMEOUT);
    EXPECT_EQ(IOT_ERROR_NOT_RADIO_COVERAGE, iot_connect(CONNECT_TIMEOUT));
}
...
TEST_F(IOTTest, connectActivatePdpTimeout)
{
    ...
    syshal_cellular_scan_ExpectAndReturn(CONNECT_TIMEOUT, SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_attach_ExpectAndReturn(CONNECT_TIMEOUT, SYSHAL_CELLULAR_NO_ERROR);
    syshal_cellular_activate_pdp_ExpectAndReturn(((syshal_cellular_apn_t*)&iot_cellular_apn.contents), CONNECT_TIMEOUT, SYSHAL_CELLULAR_ERROR_TIMEOUT);
    EXPECT_EQ(IOT_ERROR_TIMEOUT, iot_connect(CONNECT_TIMEOUT));
}
...
TEST_F(IOTTest, sendLoggingLargeStartMiddle)
{
    ...
    EXPECT_EQ(log_file.size() - last_idx, iot_send_logging(SEND_LOG_TIMEOUT, file_system, FILE_ID_LOG, last_idx));
...
TEST_F(IOTTest, sendLoggingFirstByteCorrupt)
{
    ...
    EXPECT_EQ(IOT_ERROR_FS_FILE_CORRUPTED, iot_send_logging(SEND_LOG_TIMEOUT, file_system, FILE_ID_LOG, 0));
...
TEST_F(IOTTest, downloadFileHttpsGetTimeout)
{
    ...
    EXPECT_EQ(IOT_ERROR_TIMEOUT, iot_download_file(DOWNLOAD_TIMEOUT, &url, file_system, 0, NULL));
}
