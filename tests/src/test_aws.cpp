...
public:
    ...
    bool check_json_matches_data(char * json, iot_device_status_t status)
    {
        ...
        if (status.presence_flags & IOT_LAST_LOG_FILE_READ_POS_BITMASK)
            expected.append(format("\"last_log_file_read_pos\":%lu,", status.last_log_file_read_pos));
        if (status.presence_flags & IOT_LAST_GPS_LOCATION_BITMASK)
...
TEST_F(AWSTest, jsonGetShadowFirmwareUpdate)
{
    ...
    EXPECT_EQ(IOT_FIRMWARE_UPDATE_BITMASK, shadow.device_update.presence_flags);
...
TEST_F(AWSTest, jsonGetShadowConfigurationUpdate)
{
    ...
    EXPECT_EQ(IOT_CONFIGURATION_UPDATE_BITMASK, shadow.device_update.presence_flags);
...
TEST_F(AWSTest, jsonGetShadowFull)
{
    ...
    EXPECT_EQ(IOT_CONFIGURATION_UPDATE_BITMASK | IOT_FIRMWARE_UPDATE_BITMASK, shadow.device_update.presence_flags);
...
TEST_F(AWSTest, jsonCircular)
{
    ...
    for (auto i = 0; i < 10000; ++i)
    {
        ...
        if (device_status.presence_flags & IOT_LAST_LOG_FILE_READ_POS_BITMASK)
