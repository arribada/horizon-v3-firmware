...
int syshal_timer_set(timer_handle_t handle, syshal_timer_mode_t mode, uint32_t seconds)
{
    ...
    syshal_rtc_get_timestamp((uint32_t *) &timers_priv[handle].start);
...
void syshal_timer_tick(void)
{
    ...
    for (uint32_t i = 0; i < SYSHAL_TIMER_NUMBER_OF_TIMERS; ++i)
    {
        ...
        syshal_rtc_get_timestamp(&current_time_seconds);
        ...
        if (timers_priv[i].running)
        {
            if (current_time_seconds - timers_priv[i].start >= timers_priv[i].duration)
            {
                ...
                if (periodic == timers_priv[i].mode)
                {
                    ...
                    /* Compare next timer event with this periodic timer */
                    next_timer_due = MIN(timers_priv[i].duration, next_timer_due);
                }
                else
            ...
            /* Compare the actual next timer event with the remain time in the i timer */
            else
            {
