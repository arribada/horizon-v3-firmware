...
#define SYS_CONFIG_FORMAT_VERSION (5) // This should be incremented when/if the sys_config layout is modified
...
typedef struct __attribute__((__packed__))
{
    ...
    struct __attribute__((__packed__))
    {
        uint8_t device_identifier[7];     // 7 byte Artic unique device identifier
        struct __attribute__((__packed__))
        {
            uint16_t satellite_code;      // two digit satellite code
            uint32_t seconds_since_epoch; // satellite orbital timestamp baseline
            float params[6];              // satellite orbit position
        } bulletin[8];
    } contents;
