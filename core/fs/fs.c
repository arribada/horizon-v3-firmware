    ...
    for (uint8_t i = 0; i < FS_PRIV_MAX_HANDLES; i++)
    {
        ...
        if (FS_PRIV_INVALID_INDEX == fs_priv_handle_list[index_offset].assigned_index)
        {
            ...
            *handle = curr_index;
            ...
            fs_priv_handle_list[index_offset].assigned_index = curr_index;
            curr_index = (curr_index + 1) % INT32_MIN; /* Do not permit negative numbers when converted to an int32_t */
 ...
 */
static bool get_handle_struct(fs_handle_t handle, fs_priv_handle_t **priv_handle)
{
    if (handle < 0)
        return false;
