...
uint32_t next_predict(struct bulletin_data *bulletin,  uint8_t number_sat,float lon, float lat, long minimum_time)
{
	...
	tab_PC[0].time_end = minimum_time + (24 * 60 * 60);
    ...
    uint8_t index = select_closest(pt_pp, number_sat, minimum_time);
...
//		    	printf("****** on sort juste d'une visibilite ***********\n");
	            ...
	            if (site_max < v_site_max_requis) {
	            	...
	            	// on memorise lorsque l'on quitte la visibilité
	            	duree_passage = duree;
...
uint8_t select_closest(pp *pt_pp,int number_sat, uint32_t desired_time){
	...
	for (int i = 0; i < number_sat; ++i)
	{
		...
		if(current < desired_time)
		{
			if((desired_time - current) < min )
			{
				min = (desired_time - current);
				index = i;
			}
		}
		else
		{
			if((current - desired_time) < min )
			{
