/* MS5803_05BA.c - Device driver for the MS5803_14BA pressure sensor
 *
...
int32_t MS5803_get_pressure(void)
{
    ...
    int32_t D2 = MS5803_xBA_get_adc(CMD_ADC_D2, resolution_priv);
    ...
    // dT = D2 - C5 * 2^8
    dT = D2 - ((int32_t)MS5803_xBA_coefficient[5] << 8);

    // Actual temperature
    // TEMP = 2000 + dT * C6 / 2^23
    TEMP = (((int64_t)dT * MS5803_xBA_coefficient[6]) >> 23) + 2000;

    // Offset at actual temperature
    // OFF = C2 * 2^18 + (C4 * dT ) / 2^5
    OFF = ((int64_t)MS5803_xBA_coefficient[2] << 18) + (((MS5803_xBA_coefficient[4] * (int64_t)dT)) >> 5);

    // Sensitivity at actual temperature
    // SENS = C1 * 2^17 + ( C3 * dT ) / 2^7
    SENS = ((int64_t)MS5803_xBA_coefficient[1] << 17) + (((MS5803_xBA_coefficient[3] * (int64_t)dT)) >> 7);
